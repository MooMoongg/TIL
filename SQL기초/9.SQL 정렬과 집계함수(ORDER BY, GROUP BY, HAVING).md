...
-- SQL 실행 순서: SELECT, FROM, WHERE, GROUP BY, HAVING, ORDER BY
-- 순서가 바뀌면 안됨! 반드시 암기

-- 집계된 항목에서도 필터링을 해야할 때가 있다.
-- 하지만 집계된 항목에 대해서는 WHERE을 사용할 수 없음.
-- 즉, 집계함수는 WHERE을 쓸 수 없음 → HAVING 사용

-- 실행 흐름
-- SELECT
-- FROM : 격자형 데이터 준비
-- WHERE : 필터링
-- GROUP BY : 집계 (COUNT, AVG 등)
-- HAVING : 집계된 이후 다시 필터링
-- ORDER BY : 정렬
...
-- TITLE 값이 NULL인 경우 COUNT(TITLE)로는 세지 않음.
...
-- 예제: 회원별 게시글 수 조회. 단, 게시글이 2개 이하인 레코드만 출력
SELECT WRITER_ID, COUNT(ID) AS COUNT FROM NOTICE
WHERE COUNT(ID) <= 2
GROUP BY WRITER_ID;
-- WHERE에서는 집계함수를 사용할 수 없음 → 에러 발생
